...
struct Segtree{
    ...
    void update(int l, int r, int id, int x, int y){
        ...
        int m = l+(r-l)/2;
        update(l,m,id*2,x,y);
        update(m+1,r,id*2+1,x,y);
        tree[id]= min(tree[id*2], tree[id*2+1]);
    }

    int query(int l, int r, int id, int x, int y){
        if(y< l or r<x) return inf;
        if(x<=l and r<=y) return tree[id];
        int m = l+ (r-l)/2;
        int a = query(l,m,id*2,x,y);
        int b = query(m+1,r,id*2+1,x,y);
        return min(a,b);
    }
};
...
int main() {
    ...
    while(q--){
        int x, y, z;
        cin >> x >>y >> z;
        if(x==1) sg.update(1,n,1,y,z);
        else cout << sg.query(1,n,1,y,z) << endl;;
    }
   // cout << sg.query(1,n,1,3,6);
}